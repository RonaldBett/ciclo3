function restGet() {
    $.ajax({
        url: "http://129.146.210.200:8080/api/Reservation/all",
        type: "GET",
        datatype: "json",
...
function restGetId(id,opt) {
    $.ajax({
        url: "http://129.146.210.200:8080/api/Reservation/" + id,
        type: "GET",
        datatype: "json",
        success: function (respuesta) {
            if(opt == "cli"){
...
function restPut(datos) {
    let datospeticion = JSON.stringify(datos);
    $.ajax({
        url: "http://129.146.210.200:8080/api/Reservation/update",
        data: datospeticion,
        type: "PUT",
        contentType: "application/JSON",
        success: function (respuesta) {
...
function crearCard(datos){
    let card = `<div id = ${datos.idReservation} class = "card m-2" style = "width:18rem;">`
        card +=  "<div id = 'div_card_body' class = 'card-body'>"
        card +=    "<a role='button' style = 'text-decoration: none' onclick = 'auxiliarClient(" + datos.idReservation + ")'><h5 id = 'card_title' class = 'card-title'>Cliente: "+ datos.client.name +"</h5></a>"
        card +=    `<h6 id = 'card_subtitle' class = "card-subtitle mb-2 text-muted">Fecha Inicial: ${datos.startDate} Años</h6>`
        card +=    `<h6 id = 'card_subtitle_2' class = "card-subtitle mb-2 text-muted">Fecha Final: ${datos.devolutionDate}</h6>`
        card +=    "<a role='button'  style = 'text-decoration: none' onclick = 'auxiliarCabin(" + datos.idReservation + ")'><h6 class = 'card-subtitle mb-2 text-muted'>Cabaña</h6></a>"
        card +=    "<button class = 'btn btn-dark' onclick = 'funcionBotonBorrar(" + datos.idReservation + ")'>Borrar</button> &nbsp"
        card +=   `</div>
                 </div>`;
